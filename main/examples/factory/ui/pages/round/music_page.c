...
#define MUSIC_PLAYLIST_PANEL_W UI_ROUND_POPUP_SIZE
#define MUSIC_PLAYLIST_PANEL_H UI_ROUND_POPUP_SIZE
#define MUSIC_PLAYLIST_HEADER_H 40
#define MUSIC_PLAYLIST_LIST_H 428
#define MUSIC_PLAYLIST_ROW_H 50
#define MUSIC_PLAYLIST_ROW_GAP 6
#define MUSIC_PLAYLIST_ROW_W 452
#define MUSIC_PLAYLIST_NAME_W 310
#define MUSIC_PLAYLIST_PAD_Y 6

#define MUSIC_COVER_W 200
#define MUSIC_COVER_H 200
...
#define MUSIC_SCAN_RETRY_MS 500U
#define MUSIC_PAGE_TIMER_MS 50U
...
static bool s_music_scan_pending = false;
static uint32_t s_music_scan_retry_ms = 0;

static int music_clamp_percent(int percent)
{
    if (percent < 0)
    {
        return -1;
...
static void music_spectrum_static_create(lv_obj_t *parent)
{
    ...
    lv_obj_align(spectrum, LV_ALIGN_CENTER, 0, 0);
    ...
    lv_obj_set_style_radius(spectrum, 3, 0);
...
static void music_page_update_artist_album_locked(int index)
{
    ...
    const char *name = s_music_track_ptrs[index];
...
int music_page_scan_sd_music(void)
{
    ...
    {
        ESP_LOGW(TAG, "SD storage is not available to the application");
    ...
    if (root_dir == NULL)
    {
        ...
        ESP_LOGW(TAG, "Failed to open music dir: %s, errno=%d (%s)",
                 MUSIC_SCAN_DIR, open_errno, strerror(open_errno));
        s_music_track_count = 0;
        s_music_current_index = -1;
...
static lv_obj_t *music_control_button_create(lv_obj_t *parent, const char *symbol, bool primary, audio_music_control_action_t action)
{
...
static int32_t music_playlist_panel_x_get(void)
{
    return UI_ROUND_POPUP_X;
}

static int32_t music_playlist_panel_hidden_y_get(void)
{
    int32_t root_h = s_music_ui.root ? lv_obj_get_height(s_music_ui.root) : 720;
    return root_h + 12;
}

static int32_t music_playlist_panel_target_y_get(void)
{
    return UI_ROUND_POPUP_Y;
}
...
static void music_playlist_popup_row_visual_set(lv_obj_t *row, bool current)
{
    ...
    uint32_t bg_color = current ? UI_TEXT : UI_PANEL_HL;
    uint32_t index_color = current ? UI_PRIMARY : UI_MUTED;
    uint32_t name_color = current ? UI_BG : UI_TEXT;
...
static void music_playlist_panel_create(void)
{
    ...
    lv_obj_set_pos(s_music_playlist_panel, music_playlist_panel_x_get(), music_playlist_panel_hidden_y_get());
    lv_obj_set_style_bg_color(s_music_playlist_panel, lv_color_hex(UI_PANEL), 0);
    lv_obj_set_style_bg_opa(s_music_playlist_panel, LV_OPA_70, 0);
    ...
    lv_obj_set_style_shadow_opa(s_music_playlist_panel, LV_OPA_50, 0);
...
static void music_playlist_panel_show(void)
{
    ...
    lv_obj_set_x(s_music_playlist_panel, music_playlist_panel_x_get());
    lv_obj_set_y(s_music_playlist_panel, music_playlist_panel_target_y_get());
...
static void music_playlist_panel_hide(void)
{
    ...
    lv_obj_add_flag(s_music_playlist_panel, LV_OBJ_FLAG_HIDDEN);
    lv_obj_set_y(s_music_playlist_panel, music_playlist_panel_hidden_y_get());
}
...
static lv_obj_t *music_list_button_create(lv_obj_t *parent)
{
    ...
    lv_obj_center(label);
    lv_obj_set_style_text_color(label, lv_color_hex(UI_SECONDARY), 0);
...
{
    volume_percent = music_clamp_percent(volume_percent);
        ...
        for (int i = 0; i < s_music_playlist_item_count; i++)
        {
            if (s_music_playlist_item_name[i])
            {
                lv_label_set_text(s_music_playlist_item_name[i],
                                  tracks && i < track_count && tracks[i] ? tracks[i] : "--");
            }
        }
        music_playlist_popup_refresh_current_mark();
    }
...
static lv_obj_t *msuic_create_btn(lv_obj_t *parent, int x, int y, int w, int h,
                                  const char *label, audio_music_control_action_t event_id)
{
    lv_obj_t *btn = lv_button_create(parent);
    lv_obj_set_pos(btn, x, y);
    lv_obj_set_size(btn, w, h);
    lv_obj_set_style_radius(btn, LV_RADIUS_CIRCLE, 0);
    if (event_id == MUSIC_CONTROL_LIST)
    {
        lv_obj_add_event_cb(btn, music_playlist_toggle_event_cb, LV_EVENT_SHORT_CLICKED, NULL);
    }
    else
    {
        lv_obj_add_event_cb(btn, music_control_event_cb, LV_EVENT_SHORT_CLICKED,
                            (void *)(intptr_t)event_id);
    }
    lv_obj_t *btn_label = lv_label_create(btn);
    lv_label_set_text(btn_label, label);
    lv_label_set_long_mode(btn_label, LV_LABEL_LONG_MODE_WRAP);
    lv_obj_align(btn_label, LV_ALIGN_CENTER, 0, 0);
    lv_obj_set_width(btn_label, LV_PCT(100));
    // Write style for screen_round_music_btn_1, Part: LV_PART_MAIN, State: LV_STATE_DEFAULT.
    lv_obj_set_style_bg_opa(btn, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(btn, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_radius(btn, 50, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_shadow_width(btn, 16, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_shadow_color(btn, lv_color_hex(0x3a57d7), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_shadow_opa(btn, 255, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_shadow_spread(btn, 2, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_shadow_offset_x(btn, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_shadow_offset_y(btn, 4, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(btn, lv_color_hex(0xffffff), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_font(btn, &lv_font_SourceHanSansCN_Bold_2_24, LV_PART_MAIN | LV_STATE_DEFAULT);
...
{
    (void)page;
    if (s_music_scan_pending)
    {
        s_music_scan_retry_ms += MUSIC_PAGE_TIMER_MS;
        if (s_music_scan_retry_ms >= MUSIC_SCAN_RETRY_MS)
        {
            s_music_scan_retry_ms = 0;
            s_music_scan_pending = music_page_scan_sd_music() < 0;
        }
    }
...
static void music_page_create(Page *page)
{
    ...
    s_music_ui.music_cover_img = lv_image_create(page->root);
    lv_obj_set_size(s_music_ui.music_cover_img, MUSIC_COVER_W, MUSIC_COVER_W);
    lv_obj_set_pos(s_music_ui.music_cover_img, 100, 120);
    lv_obj_set_style_radius(s_music_ui.music_cover_img, 12, 0);
    lv_obj_set_style_border_width(s_music_ui.music_cover_img, 0, 0);
    // lv_obj_set_style_border_color(s_music_ui.music_cover_img, lv_color_hex(UI_TEXT), 0);
    lv_obj_set_style_border_opa(s_music_ui.music_cover_img, LV_OPA_20, 0);
    // lv_obj_set_style_shadow_width(s_music_ui.music_cover_img, 24, 0);
    // lv_obj_set_style_shadow_spread(s_music_ui.music_cover_img, 2, 0);
    // lv_obj_set_style_shadow_color(s_music_ui.music_cover_img, lv_color_hex(UI_PRIMARY), 0);
    lv_obj_set_style_shadow_opa(s_music_ui.music_cover_img, LV_OPA_30, 0);
    lv_obj_remove_flag(s_music_ui.music_cover_img, LV_OBJ_FLAG_CLICKABLE);
    music_cover_apply_locked();

    s_music_lyric_layer = lv_obj_create(page->root);
    lv_obj_set_size(s_music_lyric_layer, 300, 200);
    lv_obj_set_pos(s_music_lyric_layer, 320, 120);
    ui_obj_set_transparent(s_music_lyric_layer);
    lv_obj_set_style_bg_color(s_music_lyric_layer, lv_color_hex(UI_PANEL_HL), 0);
    lv_obj_set_style_bg_opa(s_music_lyric_layer, LV_OPA_30, 0);
    lv_obj_set_style_border_width(s_music_lyric_layer, 0, 0);
    // lv_obj_set_style_border_color(s_music_lyric_layer, lv_color_hex(UI_TEXT), 0);
    // lv_obj_set_style_border_opa(s_music_lyric_layer, LV_OPA_20, 0);
    lv_obj_set_style_radius(s_music_lyric_layer, 22, 0);
    // lv_obj_set_style_shadow_width(s_music_lyric_layer, 20, 0);
    // lv_obj_set_style_shadow_color(s_music_lyric_layer, lv_color_hex(UI_SECONDARY), 0);
    // lv_obj_set_style_shadow_opa(s_music_lyric_layer, LV_OPA_20, 0);
    ...
    lv_obj_t *volume_cont = lv_obj_create(page->root);
    lv_obj_set_pos(volume_cont, 205, 342);
    lv_obj_set_size(volume_cont, 310, 42);
    lv_obj_set_style_bg_opa(volume_cont, LV_OPA_TRANSP, 0);
    lv_obj_set_style_border_width(volume_cont, 0, 0);
    lv_obj_set_style_pad_all(volume_cont, 0, 0);
    lv_obj_set_scrollbar_mode(volume_cont, LV_SCROLLBAR_MODE_OFF);
    lv_obj_remove_flag(volume_cont, LV_OBJ_FLAG_SCROLLABLE);

    lv_obj_t *volume_icon = lv_label_create(volume_cont);
    lv_obj_set_pos(volume_icon, 0, 8);
    lv_obj_set_size(volume_icon, 28, 26);
    lv_label_set_text(volume_icon, LV_SYMBOL_VOLUME_MID);
    lv_obj_set_style_text_font(volume_icon, &lv_font_montserrat_20, 0);
    lv_obj_set_style_text_color(volume_icon, lv_color_hex(UI_MUTED), 0);
    lv_obj_set_style_text_align(volume_icon, LV_TEXT_ALIGN_CENTER, 0);

    s_music_ui.volume_slider = lv_slider_create(volume_cont);
    lv_obj_set_pos(s_music_ui.volume_slider, 42, 17);
    lv_obj_set_size(s_music_ui.volume_slider, 210, 8);
    ...
    lv_obj_set_style_bg_opa(s_music_ui.volume_slider, LV_OPA_70, LV_PART_MAIN);
    ...
    lv_obj_set_style_width(s_music_ui.volume_slider, 20, LV_PART_KNOB);
    lv_obj_set_style_height(s_music_ui.volume_slider, 20, LV_PART_KNOB);
    lv_obj_set_style_border_width(s_music_ui.volume_slider, 0, LV_PART_KNOB);
    lv_obj_set_style_shadow_color(s_music_ui.volume_slider, lv_color_hex(UI_PRIMARY), LV_PART_KNOB);
    lv_obj_set_style_shadow_width(s_music_ui.volume_slider, 8, LV_PART_KNOB);
    ...
    lv_obj_set_pos(s_music_ui.volume_value_label, 264, 11);
    lv_obj_set_size(s_music_ui.volume_value_label, 46, 20);
    lv_label_set_text_fmt(s_music_ui.volume_value_label, "%d%%", s_music_volume_percent);
    lv_obj_set_style_text_font(s_music_ui.volume_value_label, &lv_font_montserrat_14, 0);
    lv_obj_set_style_text_color(s_music_ui.volume_value_label, lv_color_hex(UI_MUTED), 0);
    lv_obj_set_style_text_align(s_music_ui.volume_value_label, LV_TEXT_ALIGN_RIGHT, 0);

    s_music_ui.song_name_label = lv_label_create(page->root);
    lv_obj_set_pos(s_music_ui.song_name_label, 230, 400);
    lv_obj_set_size(s_music_ui.song_name_label, 260, 32);
    lv_label_set_text(s_music_ui.song_name_label, "------");
    lv_label_set_long_mode(s_music_ui.song_name_label, LV_LABEL_LONG_MODE_DOTS);
    lv_obj_set_style_text_font(s_music_ui.song_name_label, MUSIC_NAME_FONT, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_align(s_music_ui.song_name_label, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(s_music_ui.song_name_label, lv_color_hex(UI_TEXT), LV_PART_MAIN | LV_STATE_DEFAULT);

    s_music_ui.artist_album_label = lv_label_create(page->root);
    lv_obj_set_pos(s_music_ui.artist_album_label, 230, 470);
    lv_obj_set_size(s_music_ui.artist_album_label, 260, 32);
    lv_label_set_text(s_music_ui.artist_album_label, "artist / album");
    lv_label_set_long_mode(s_music_ui.artist_album_label, LV_LABEL_LONG_MODE_DOTS);
    lv_obj_set_style_text_font(s_music_ui.artist_album_label, MUSIC_NAME_FONT, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_align(s_music_ui.artist_album_label, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(s_music_ui.artist_album_label, lv_color_hex(UI_TEXT), LV_PART_MAIN | LV_STATE_DEFAULT);

    // Write codes screen_round_music_slider_1
    s_music_ui.paly_progress_bar = lv_slider_create(page->root);
    lv_obj_set_pos(s_music_ui.paly_progress_bar, 60, 440);
    lv_obj_set_size(s_music_ui.paly_progress_bar, 600, 8);
    lv_slider_set_range(s_music_ui.paly_progress_bar, 0, 100);
    lv_slider_set_mode(s_music_ui.paly_progress_bar, LV_SLIDER_MODE_NORMAL);
    lv_slider_set_value(s_music_ui.paly_progress_bar, 10, LV_ANIM_OFF);

    // Write style for screen_round_music_slider_1, Part: LV_PART_MAIN, State: LV_STATE_DEFAULT.
    lv_obj_set_style_bg_opa(s_music_ui.paly_progress_bar, 60, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(s_music_ui.paly_progress_bar, lv_color_hex(0x2195f6), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_grad_dir(s_music_ui.paly_progress_bar, LV_GRAD_DIR_NONE, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_radius(s_music_ui.paly_progress_bar, 8, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_outline_width(s_music_ui.paly_progress_bar, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_shadow_width(s_music_ui.paly_progress_bar, 0, LV_PART_MAIN | LV_STATE_DEFAULT);

    // Write style for screen_round_music_slider_1, Part: LV_PART_INDICATOR, State: LV_STATE_DEFAULT.
    lv_obj_set_style_bg_opa(s_music_ui.paly_progress_bar, 255, LV_PART_INDICATOR | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(s_music_ui.paly_progress_bar, lv_color_hex(0x3a57d7), LV_PART_INDICATOR | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_grad_dir(s_music_ui.paly_progress_bar, LV_GRAD_DIR_NONE, LV_PART_INDICATOR | LV_STATE_DEFAULT);
    lv_obj_set_style_radius(s_music_ui.paly_progress_bar, 8, LV_PART_INDICATOR | LV_STATE_DEFAULT);

    // Write style for screen_round_music_slider_1, Part: LV_PART_KNOB, State: LV_STATE_DEFAULT.
    lv_obj_set_style_bg_opa(s_music_ui.paly_progress_bar, 0, LV_PART_KNOB | LV_STATE_DEFAULT);
    lv_obj_set_style_radius(s_music_ui.paly_progress_bar, 8, LV_PART_KNOB | LV_STATE_DEFAULT);

    s_music_ui.paly_progress_current_time_label = lv_label_create(page->root);
    lv_obj_set_pos(s_music_ui.paly_progress_current_time_label, 60, 450);
    lv_obj_set_size(s_music_ui.paly_progress_current_time_label, 70, 20);
    lv_label_set_text(s_music_ui.paly_progress_current_time_label, "00:00");
    lv_label_set_long_mode(s_music_ui.paly_progress_current_time_label, LV_LABEL_LONG_WRAP);
    lv_obj_set_style_text_align(s_music_ui.paly_progress_current_time_label, LV_TEXT_ALIGN_LEFT, LV_PART_MAIN | LV_STATE_DEFAULT);

    s_music_ui.paly_progress_total_time_label = lv_label_create(page->root);
    lv_obj_set_pos(s_music_ui.paly_progress_total_time_label, 590, 450);
    lv_obj_set_size(s_music_ui.paly_progress_total_time_label, 70, 20);
    lv_label_set_text(s_music_ui.paly_progress_total_time_label, "--:--");
    lv_label_set_long_mode(s_music_ui.paly_progress_total_time_label, LV_LABEL_LONG_WRAP);
    lv_obj_set_style_text_align(s_music_ui.paly_progress_total_time_label, LV_TEXT_ALIGN_RIGHT, LV_PART_MAIN | LV_STATE_DEFAULT);

    msuic_create_btn(page->root, 100, 530, 80, 80, LV_SYMBOL_PREV, MUSIC_CONTROL_PREV);
    s_music_ui.play_btn_label = msuic_create_btn(page->root, 320, 530, 80, 80,
                                                  LV_SYMBOL_PLAY,
                                                  MUSIC_CONTROL_PLAY_PAUSE);
    msuic_create_btn(page->root, 540, 530, 80, 80, LV_SYMBOL_NEXT, MUSIC_CONTROL_NEXT);
    s_music_ui.play_mode_btn_label = msuic_create_btn(page->root, 220, 610, 80, 80,
                                                       music_play_mode_symbol(s_music_play_mode),
                                                       MUSIC_CONTROL_PLAY_MODE);
    msuic_create_btn(page->root, 410, 610, 80, 80, LV_SYMBOL_BARS, MUSIC_CONTROL_LIST);
    music_playlist_panel_create();
...
void music_page_register(void)
{
    Page page = {
        ...
        .timer_interval_ms = MUSIC_PAGE_TIMER_MS,
    };
