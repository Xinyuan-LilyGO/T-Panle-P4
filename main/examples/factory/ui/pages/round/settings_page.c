...
#define SET_PAGE_MAIN_W UI_ROUND_MAIN_CONT_W
#define SET_PAGE_MAIN_H UI_ROUND_MAIN_CONT_H
#define SET_PAGE_MAIN_Y UI_ROUND_MAIN_CONT_Y
...
#define SET_PAGE_OPTION_FAR_W 480
#define SET_PAGE_OPTION_TEXT_W 350
#define SET_PAGE_DETAIL_W UI_ROUND_POPUP_SIZE
#define SET_PAGE_DETAIL_H UI_ROUND_POPUP_SIZE
#define SET_PAGE_DETAIL_Y 0
#define SET_PAGE_DETAIL_BODY_H 408
#define SET_PAGE_PMU_PANEL_H 400
#define SET_PAGE_DETAIL_TITLE_W 340
#define SET_PAGE_DETAIL_TEXT_W 408
#define SET_WIFI_DIALOG_W UI_ROUND_POPUP_SIZE
#define SET_WIFI_DIALOG_H UI_ROUND_POPUP_SIZE
#define SET_WIFI_DIALOG_Y 0
#define SET_WIFI_LIST_H 410
#define SET_WIFI_ROW_SSID_W 238
#define SET_WIFI_ROW_RSSI_W 70
#define SET_WIFI_ROW_TYPE_W 88
#define SET_WIFI_DIALOG_TITLE_W 142
#define SET_WIFI_STATUS_W 188
#define SET_WIFI_PASSWORD_W UI_ROUND_POPUP_SIZE
#define SET_WIFI_PASSWORD_H UI_ROUND_POPUP_SIZE
#define SET_WIFI_PASSWORD_Y 0
#define SET_WIFI_PASSWORD_SSID_W 306
#define SET_WIFI_PASSWORD_TA_W 336
#define SET_WIFI_KEYBOARD_H 220
...
static void set_page_power_confirm_cancel_event_cb(lv_event_t *e)
{
...
static void set_page_power_confirm_ok_event_cb(lv_event_t *e)
{
    if (lv_event_get_code(e) != LV_EVENT_CLICKED)
    {
        return;
...
static void set_page_power_confirm_show(set_power_action_t action)
{
    ...
    lv_obj_set_style_bg_color(cancel_btn, lv_color_hex(UI_PANEL_HL), 0);
    lv_obj_set_style_bg_opa(cancel_btn, LV_OPA_70, 0);
    lv_obj_set_style_radius(cancel_btn, 21, 0);
    lv_obj_set_style_border_width(cancel_btn, 1, 0);
    lv_obj_set_style_border_color(cancel_btn, lv_color_hex(UI_TEXT), 0);
    lv_obj_set_style_border_opa(cancel_btn, LV_OPA_30, 0);
    lv_obj_add_event_cb(cancel_btn, set_page_power_confirm_cancel_event_cb,
                        LV_EVENT_CLICKED, NULL);
    lv_obj_t *cancel_label = ui_label_create(cancel_btn, "CANCEL",
                                             &lv_font_montserrat_14, UI_TEXT);
    lv_obj_center(cancel_label);

    lv_obj_t *confirm_btn = lv_button_create(buttons);
    lv_obj_set_size(confirm_btn, 150, 42);
    lv_obj_set_style_bg_color(confirm_btn,
                              lv_color_hex(ship_mode ? UI_WARN : UI_PRIMARY), 0);
    lv_obj_set_style_bg_opa(confirm_btn, LV_OPA_COVER, 0);
    lv_obj_set_style_radius(confirm_btn, 21, 0);
    lv_obj_set_style_border_width(confirm_btn, 0, 0);
    lv_obj_add_event_cb(confirm_btn, set_page_power_confirm_ok_event_cb,
                        LV_EVENT_CLICKED, (void *)(intptr_t)action);
    lv_obj_t *confirm_label = ui_label_create(confirm_btn, "CONFIRM",
                                              &lv_font_montserrat_14, UI_BG);
...
static void set_page_wifi_password_dialog_create(void)
{
    ...
    lv_obj_align(s_set_ui.wifi_password_dialog, LV_ALIGN_CENTER, 0, SET_WIFI_PASSWORD_Y);
...
static void set_page_wifi_dialog_create(void)
{
    ...
    lv_obj_align(s_set_ui.wifi_dialog, LV_ALIGN_CENTER, 0, SET_WIFI_DIALOG_Y);
...
static lv_obj_t *set_page_detail_begin(const char *icon, const char *title, uint32_t color)
{
    ...
    {
        s_set_ui.settings_detail_dialog = lv_obj_create(s_set_ui.root);
    ...
    lv_obj_align(s_set_ui.settings_detail_dialog, LV_ALIGN_CENTER, 0, SET_PAGE_DETAIL_Y);
    ...
    lv_obj_clear_flag(s_set_ui.settings_detail_dialog, LV_OBJ_FLAG_HIDDEN);
...
static void set_page_wifi_detail_show(void)
{
    ...
    lv_obj_set_style_radius(wifi_info, 10, 0);
...
static void set_page_bluetooth_detail_show(void)
{
    lv_obj_t *body = set_page_detail_begin(LV_SYMBOL_BLUETOOTH, "BLE", UI_SECONDARY);
    status_info_t status = {0};
    bool status_ok = status_bar_info_get(&status);

    lv_obj_t *status_panel = set_page_panel_create(body, LV_PCT(100), 142, "BLE STATUS", UI_SECONDARY);
    set_page_info_line_create(status_panel,
                              "ESP32-C5",
                              esp32c5_sdio_slave_is_sleeping() ? "SLEEP" :
                              (esp32c5_sdio_slave_is_ready() ? "READY" : "NOT READY"),
                              esp32c5_sdio_slave_is_ready() ? UI_OK : UI_ERROR);
    set_page_info_line_create(status_panel,
                              "BLE Host",
                              (status_ok && status.bluetooth_enabled) ? "ON" : "HOST OFF",
                              (status_ok && status.bluetooth_enabled) ? UI_OK : UI_WARN);
    set_page_info_line_create(status_panel,
                              "Connection",
                              (status_ok && status.bluetooth_connected) ? "CONNECTED" : "IDLE",
                              (status_ok && status.bluetooth_connected) ? UI_PRIMARY : UI_MUTED);

    lv_obj_t *cap_panel = set_page_panel_create(body, LV_PCT(100), 214, "BLE CAPABILITY", UI_SECONDARY);
    set_page_info_line_create(cap_panel, "Radio", "ESP32-C5", UI_TEXT);
    set_page_info_line_create(cap_panel, "Protocol", "BLE only", UI_SECONDARY);
    set_page_info_line_create(cap_panel, "Classic BT", "NOT SUPPORTED", UI_WARN);
    set_page_info_line_create(cap_panel, "Role", "Peripheral / GATT", UI_PRIMARY);
    set_page_info_line_create(cap_panel, "Profile", "Factory service", UI_TEXT);
    set_page_info_line_create(cap_panel, "Control", "Status only", UI_WARN);
}
...
static void set_page_display_detail_show(void)
{
    ...
    lv_obj_t *timeout_row = ui_flex_container_create(display_panel, LV_PCT(100), 46,
                                                     LV_FLEX_FLOW_ROW,
                                                     LV_FLEX_ALIGN_START,
                                                     LV_FLEX_ALIGN_CENTER,
                                                     LV_FLEX_ALIGN_CENTER);
...
static void set_page_detail_option_event_cb(lv_event_t *e)
{
    ...
    case SET_DETAIL_WIFI:
        ...
        break;
    case SET_DETAIL_BLUETOOTH:
        set_page_bluetooth_detail_show();
        break;
...
static void set_page_create(Page *page)
{
    ...
    lv_obj_set_size(s_set_ui.main_cont, SET_PAGE_MAIN_W, SET_PAGE_MAIN_H);
    lv_obj_set_pos(s_set_ui.main_cont, UI_ROUND_MAIN_CONT_X, SET_PAGE_MAIN_Y);
    ...
    lv_obj_set_size(options_panel, SET_PAGE_MAIN_W, SET_PAGE_MAIN_H);
    lv_obj_set_pos(options_panel, 0, 0);
    ...
    lv_obj_remove_flag(options_panel, LV_OBJ_FLAG_SCROLLABLE);
