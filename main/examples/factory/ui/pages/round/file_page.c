...
#define FILE_ROUND_CONTENT_W UI_ROUND_MAIN_CONT_W
#define FILE_ROUND_PANEL_H UI_ROUND_MAIN_CONT_H
#define FILE_ROUND_PATH_H 40
#define FILE_ROUND_LIST_H 434
#define FILE_ROUND_ROW_H 48
#define FILE_ROUND_NAME_W 300
#define FILE_ROUND_TYPE_W 68
#define FILE_ROUND_VIEWER_SIZE UI_ROUND_POPUP_SIZE
#define FILE_ROUND_VIEWER_BODY_H 430
#define FILE_ROUND_VIEWER_STAGE_W 456
#define FILE_ROUND_VIEWER_STAGE_H 406
...
static void file_page_image_show_scaled(uint32_t width, uint32_t height)
{
    ...
    const uint32_t stage_w = FILE_ROUND_VIEWER_STAGE_W;
    const uint32_t stage_h = FILE_ROUND_VIEWER_STAGE_H;
...
static void file_page_add_row(lv_obj_t *parent, int index, const char *name, bool is_dir, const char *event_name)
{
    ...
    lv_obj_set_width(name_label, FILE_ROUND_NAME_W);
    ...
    lv_obj_set_width(type_label, FILE_ROUND_TYPE_W);
...
static int file_page_scan_dir(const char *dir_path)
{
    ...
    if (!storage_app_access_begin())
    {
        ...
        {
            lv_label_set_text(s_file_ui.count_label, "SD UNAVAILABLE");
...
static void file_page_viewer_create(lv_obj_t *parent)
{
    ...
    lv_obj_set_size(s_file_ui.viewer_cont, FILE_ROUND_VIEWER_SIZE, FILE_ROUND_VIEWER_SIZE);
    lv_obj_set_pos(s_file_ui.viewer_cont, UI_ROUND_POPUP_X, UI_ROUND_POPUP_Y);
    file_page_glass_style(s_file_ui.viewer_cont, UI_PANEL, LV_OPA_70,
                          UI_TEXT, LV_OPA_30, UI_PRIMARY, LV_OPA_30, 24, 22);
    lv_obj_set_style_pad_all(s_file_ui.viewer_cont, 12, 0);
    ...
    lv_obj_set_width(s_file_ui.viewer_title_label, 220);
    ...
    lv_obj_set_width(s_file_ui.viewer_status_label, 140);
    ...
    lv_obj_set_size(s_file_ui.viewer_body, LV_PCT(100), FILE_ROUND_VIEWER_BODY_H);
...
static void file_page_create(Page *page)
{
    ...
    lv_obj_set_style_pad_left(path_row, 10, 0);
    lv_obj_set_style_pad_right(path_row, 10, 0);
    ...
    lv_obj_set_width(s_file_ui.path_label, 300);
    ...
    lv_obj_set_size(s_file_ui.list_cont, LV_PCT(100), FILE_ROUND_LIST_H);
    ...
    lv_obj_set_style_pad_row(s_file_ui.list_cont, 6, 0);
