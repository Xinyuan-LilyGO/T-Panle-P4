...
#define ROUND_LORA_CONTENT_W 640
#define ROUND_LORA_CHAT_H 480
                                     ...
                                     const char *message, void *user_data)
{
    ...
    const char *mode_text = mode == FACTORY_LORA_MODE_RX ? "RX" : mode == FACTORY_LORA_MODE_TX || mode == FACTORY_LORA_MODE_TX_ONCE ? "TX"
                                                                                                                                    : "STBY";
    uint32_t color = state == 0 ? (mode == FACTORY_LORA_MODE_RX ? UI_OK : mode == FACTORY_LORA_MODE_STOP ? UI_MUTED
                                                                                                         : UI_PRIMARY)
                                : UI_ERROR;
...
static void round_lora_params_changed(void)
{
    ...
    lv_label_set_text_fmt(s_round_lora_ui.parameter_summary,
                          "Config: %.1f MHz  BW %.1f  SF%d  4/%d  %d dBm",
...
static void round_lora_keyboard_create(lv_obj_t *root)
{
    ...
    lv_obj_set_size(s_round_lora_ui.keyboard, 640, 270);
    ...
    lv_obj_set_style_bg_opa(s_round_lora_ui.keyboard, LV_OPA_70, LV_PART_ITEMS);
    lv_obj_set_style_border_width(s_round_lora_ui.keyboard, 1, LV_PART_ITEMS);
    lv_obj_set_style_border_color(s_round_lora_ui.keyboard, lv_color_hex(UI_TEXT), LV_PART_ITEMS);
    lv_obj_set_style_border_opa(s_round_lora_ui.keyboard, LV_OPA_20, LV_PART_ITEMS);
    lv_obj_set_style_radius(s_round_lora_ui.keyboard, 10, LV_PART_ITEMS);
...
static void round_lora_parameter_dialog_create(lv_obj_t *root)
{
    ...
    lv_obj_set_size(panel, 680, 560);
    lv_obj_align(panel, LV_ALIGN_CENTER, 0, 20);
    round_lora_panel_style(panel, UI_PANEL, 24);
...
static void lora_page_create(Page *page)
{
    ...
    lv_obj_t *status_strip = ui_flex_container_create(main_cont, ROUND_LORA_CONTENT_W, 48,
                                                      ...
                                                      LV_FLEX_ALIGN_CENTER,
                                                      LV_FLEX_ALIGN_CENTER);
    round_lora_panel_style(status_strip, UI_PANEL, 24);
    ...
    lv_obj_set_width(s_lora_ui.lora_log, 90);
    ...
    lv_obj_set_width(s_round_lora_ui.parameter_summary, 560);
    ...
    round_lora_panel_style(chat_panel, UI_PANEL, 24);
    ...
    lv_obj_t *chat_head = ui_flex_container_create(chat_panel, LV_PCT(100), 42,
                                                   ...
                                                   LV_FLEX_ALIGN_CENTER,
                                                   LV_FLEX_ALIGN_CENTER);
    lv_obj_t *messages_title = ui_label_create(chat_head, "CHAT", &lv_font_montserrat_12, UI_SECONDARY);
    ...
    lv_obj_set_size(s_round_lora_ui.chat_list, LV_PCT(100), 332);
    ...
    lv_obj_t *composer_row = ui_flex_container_create(main_cont, ROUND_LORA_CONTENT_W, 60,
                                                      LV_FLEX_FLOW_ROW,
                                                      LV_FLEX_ALIGN_START,
                                                      LV_FLEX_ALIGN_CENTER,
                                                      LV_FLEX_ALIGN_CENTER);
    lv_obj_set_style_pad_column(composer_row, 8, 0);
    lv_obj_set_style_border_width(composer_row, 0, 0);
    s_round_lora_ui.composer = lora_textarea_create(composer_row, 590, 52, "", true);
    s_lora_ui.tx_input = s_round_lora_ui.composer;
    lv_textarea_set_placeholder_text(s_round_lora_ui.composer, "input text and send");
    lv_obj_add_event_cb(s_round_lora_ui.composer, round_lora_composer_event_cb, LV_EVENT_CLICKED, NULL);
    round_lora_icon_button_create(composer_row, LV_SYMBOL_RIGHT, UI_PRIMARY, round_lora_send_event_cb);
...
static void round_lora_page_on_timer(void)
{
    ...
    if (control_mode == s_round_lora_ui.last_control_mode)
    {
        if (s_round_lora_ui.periodic_btn_label)
        {
            lv_label_set_text_fmt(s_round_lora_ui.periodic_btn_label,
                                  "%s  LOOP SEND %ds", LV_SYMBOL_REFRESH, s_lora_tx_interval_s);
        }
        return;
    }
    ...
    lv_label_set_text_fmt(s_round_lora_ui.periodic_btn_label, "%s  LOOP SEND %ds",
