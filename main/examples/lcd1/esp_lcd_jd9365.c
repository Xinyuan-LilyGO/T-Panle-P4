...
static esp_err_t panel_jd9365_init(esp_lcd_panel_t *panel)
{
    ...
    if (jd9365->init_cmds) {
        ...
        ESP_LOGE(TAG, "No init commands provided, skipping initialization");
    }

    for (int i = 0; i < init_cmds_size; i++) {
        const void *cmd_data = init_cmds[i].data;
        ...
        if (is_user_set && (init_cmds[i].data_bytes > 0)) {
            ...
            case LCD_CMD_MADCTL:
                ...
                madctl_data = (((const uint8_t *)init_cmds[i].data)[0] & ~LCD_CMD_BGR_BIT) |
                               (jd9365->madctl_val & LCD_CMD_BGR_BIT);
                jd9365->madctl_val = madctl_data;
            ...
            if (is_cmd_overwritten) {
                is_cmd_overwritten = false;
                ESP_LOGW(TAG, "The %02Xh command conflicts with the panel configuration",
                         init_cmds[i].cmd);
            }
        }

        // Send command
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, init_cmds[i].cmd, cmd_data, init_cmds[i].data_bytes), TAG, "send command failed");
