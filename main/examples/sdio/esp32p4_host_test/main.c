...
#endif
    config.input_delay_phase = SDMMC_DELAY_PHASE_0;
...
static esp_err_t set_uart_mux(bool to_c5)
{
    const uint32_t pin_mask = BIT(XL9555_UART_SEL);

    if (!to_c5) {
        ESP_RETURN_ON_ERROR(esp_io_expander_set_level(s_expander, pin_mask, 0),
    ...
    ESP_RETURN_ON_ERROR(esp_io_expander_set_level(s_expander, pin_mask, 1),
                        TAG, "switch UART mux to C5 failed");
    s_uart_to_c5 = true;
    return ESP_OK;
}
...
static esp_err_t init_board_io(void)
{
    ...
    ESP_RETURN_ON_ERROR(set_uart_mux(false), TAG, "set default UART mux failed");
