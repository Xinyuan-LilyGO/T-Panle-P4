...
{
    memset(handle, 0, sizeof(touch_handle_t));
    ...
    touch_soft_reset(handle);

    handle->touch_info_addr = TOUCH_DATA_ADDR;

    uint8_t chip_id[2] = {0};
    esp_err_t ret = touch_get_chip_id(handle, chip_id);
    if (ret == ESP_OK)
    {
    ...
    touch_get_resolution(handle, &x_res, &y_res);

    uint8_t fw_version[8];
    touch_get_fw_version(handle, fw_version);
...
esp_err_t touch_get_single_point(touch_handle_t *handle,
                                 ...
                                 uint8_t finger_num)
{
    if (finger_num == 0 || finger_num > MAX_TOUCH_POINTS)
    {
                                    ...
                                    touch_data_t *data)
{
    ...
    const uint8_t total_size = TOUCH_POINT_ADDR_OFFSET +
                               MAX_TOUCH_POINTS * SINGLE_TOUCH_POINT_SIZE;
    uint8_t buf[TOUCH_POINT_ADDR_OFFSET + MAX_TOUCH_POINTS * SINGLE_TOUCH_POINT_SIZE] = {0};
    ...
    if (buf[0] == 0 || buf[0] > MAX_TOUCH_POINTS)
    {
        return ESP_ERR_NOT_FOUND;
    }
